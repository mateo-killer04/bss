...
window.dialogue_Mateo=function(player,items){
    
    let addCommas=(s)=>{for(let i=s.length-3;i>0;i-=3){s=s.substring(0,i)+','+s.substr(i,s.length)}return s},doGrammar=(s)=>{let str=s.slice(),_s='';for(let i in str){if(str[i].toLowerCase()!==str[i]){_s=_s+' '+str[i]}else{_s=_s+str[i]}}return _s[0].toUpperCase()+_s.substring(1,_s.length)},addReward=(arr)=>{
        
        for(let i in player.quests){
            
            if(player.quests[i].NPC==='Mateo') player.quests.splice(i,1)
        ...
        for(let i in arr){
            ...
            player.addMessage('+'+addCommas(arr[i][1]+'')+' '+doGrammar(arr[i][0])+' (from Mateo)')
        }
        
        player.updateInventory()
    }
    
    return ["Hi! I'm Mateo, the creator of this game!","In this questline, I'll reward some Star Eggs for every quest!","I'll also reward you with a Star Treat at the last quest!","So here's the first quest:",function(){player.addQuest('Programs',[['redPollen',30000000],['pollenFromRoseField',10000000],['pollenFromStrawberryField',10000000],['pollenFromMushroomField',10000000],['flames',30],['strawberryTokens',25],['strawberry',50],['redExtractTokens',2],['redExtract',3],['timesUsingTheRedCannon',25],['redAbilityTokens',250],['scorpion',15],['werewolf',3]],'Mateo');},"Great! Here's your Star Egg!","Hatch it to get a gifted bee you don't have in your hive!",function(){addReward([['honey',75000000],['starEgg',1],['redExtract',3],['ticket',5],['treat',1000],['royalJelly',25],['invigoratingVial',1]])},"Here's your next quest, good luck!",function(){player.addQuest('Locations',[['pollen',75000000],['goo',10000000],['pollenFromPumpkinPatch',15000000],['pollenFromCactusField',15000000],['pollenFromCloverField',10000000],['pollenFromSunflowerField',7500000],['royalJellyTokens',5],['boostTokens',500],['tokensFromPlanters',30],['rhinoBeetle',15],['ladybug',15]],'Mateo');},"Good!",function(){addReward([['honey',100000000],['starEgg',1],['fieldDice',5],['glitter',3],['treat',2500],['ticket',5],['royalJelly',50]])},"Here's the next quest!",function(){player.addQuest('Buffers',[['whitePollen',100000000],['pollenFromPineapplePatch',30000000],['pollenFromDandelionField',10000000],['pollenFromSunflowerField',10000000],['bombTokens',500],['markTokens',250],['tokensFromSprouts',50],['sunflowerSeedTokens',25],['pineappleTokens',25],['timesUsingTheSlingshot',25],['spider',5]],'Mateo');},"Impressive!",function(){addReward([['honey',100000000],['starEgg',1],['enzymes',5],['oil',5],['jellyBeans',10],['glue',10]])},"Just 2 more quests to go!",function(){player.addQuest('Pointers',[['pollenFromAntField',10000000],['ant',500],['armyAnt',50],['fireAnt',50],['flyingAnt',50],['giantAnt',10],['tunnelBear',1],['kingBeetle',2],['rogueViciousBee',3],['werewolf',5],['spider',10],['mantis',20],['scorpion',20],['battleTokens',500]],'Mateo');},"Great! Your next quest will be harder, so good luck!",function(){addReward([['honey',250000000],['starEgg',1],['stinger',10],['purplePotion',1],['loadedDice',1],['treat',3500]])},"This is the last quest, complete it for a star treat!",function(){player.addQuest('Draw Call',[['pollen',1000000000],['bluePollen',750000000],['bubbles',150],['blueAbilityTokens',1000],['hasteTokens',500],['tokensFromSprouts',100],['timesUsingTheBlueCannon',40],['tropicalDrink',3],['fallingCoconuts',35],['mantis',15],['rhinoBeetle',30],['rogueViciousBee',3]],'Mateo');},"Epic! You're the best beekeeper I've ever met! You deserve this Star Treat!",function(){addReward([['honey',2500000000],['starTreat',1],['blueExtract',5],['royalJelly',250],['ticket',15],['treat',5270],['blueberry',2],['magicBean',10]])},"You did it!","All my quests are done and you've earned your star treat!","sad that i only fit 8 references into my NPC's short questlines :(","ok bye"]
}

window.dialogue_Mateo_beesmas=function(player,items){
    
    let addCommas=(s)=>{for(let i=s.length-3;i>0;i-=3){s=s.substring(0,i)+','+s.substr(i,s.length)}return s},doGrammar=(s)=>{let str=s.slice(),_s='';for(let i in str){if(str[i].toLowerCase()!==str[i]){_s=_s+' '+str[i]}else{_s=_s+str[i]}}return _s[0].toUpperCase()+_s.substring(1,_s.length)},addReward=(arr)=>{
        
        for(let i in player.quests){
            
            if(player.quests[i].NPC==='Mateo'&&player.quests[i].isBeesmasQuest) player.quests.splice(i,1)
